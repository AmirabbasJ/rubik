...
(function () {
  ...
  function messageHandler(event) {
    if (event.data.type == 'generateTables') {
      ...
      var x = Search.solution(
        event.data.cube,
        event.data.maxDepth || 20,
  ...
  FaceCube.prototype.toCubieCube = function () {
    ...
    for (var i = 0; i < 8; i++) ccRet.cp[i] = URF; // invalidate corners
    for (var i = 0; i < 12; i++) ccRet.ep[i] = UR; // and edges
    var col1, col2;
    for (var i in Corner) {
      i = Corner[i];
      ...
      for (ori = 0; ori < 3; ori++)
        if (
          this.f[cornerFacelet[i.ordinal()][ori].ordinal()] == U ||
          this.f[cornerFacelet[i.ordinal()][ori].ordinal()] == D
        )
          break;
      ...
      for (var j in Corner) {
        j = Corner[j];
        if (
          col1 == cornerColor[j.ordinal()][1] &&
          col2 == cornerColor[j.ordinal()][2]
        ) {
    ...
    for (var i in Edge) {
      ...
      for (var j in Edge) {
        j = Edge[j];
        if (
          this.f[edgeFacelet[i.ordinal()][0].ordinal()].toString() ==
            edgeColor[j.ordinal()][0].toString() &&
          this.f[edgeFacelet[i.ordinal()][1].ordinal()].toString() ==
            edgeColor[j.ordinal()][1].toString()
        ) {
  ...
  CubieCube.prototype.rotateLeft = function (
    arr,
    l,
    r // Left rotation of all array elements between l and r
  ) {
    var temp = arr[l];
    for (var i = l; i < r; i++) arr[i] = arr[i + 1];
    arr[r] = temp;
  };
  CubieCube.prototype.rotateRight = function (
    arr,
    l,
    r // Right rotation of all array elements between l and r
  ) {
    var temp = arr[r];
  ...
  CubieCube.prototype.edgeMultiply = function (b) {
    ...
    for (var edge in Edge) {
      ...
      eOri[edge.ordinal()] =
        (b.eo[edge.ordinal()] + this.eo[b.ep[edge.ordinal()].ordinal()]) % 2;
    }
    for (var e in Edge) {
      e = Edge[e];
  ...
  CubieCube.prototype.getFRtoBR = function () {
    ...
    for (var j = BR.ordinal(); j >= UR.ordinal(); j--)
      if (
        FR.ordinal() <= this.ep[j].ordinal() &&
        this.ep[j].ordinal() <= BR.ordinal()
      ) {
        a += Cnk(11 - j, x + 1);
        edge4[3 - x++] = this.ep[j];
      }
    ...
    for (
      var j = 3;
      j > 0;
      j-- // compute the index b < 4! for the // permutation in perm
    ) {
      var k = 0;
  ...
  CubieCube.prototype.setFRtoBR = function (idx) {
    ...
    for (
      var j = 1, k;
      j < 4;
      j++ // generate permutation from index b
    ) {
      k = b % (j + 1);
      b /= j + 1;
      b = b | 0;
  ...
  CubieCube.prototype.getURFtoDLF = function () {
    ...
    for (
      var j = 5;
      j > 0;
      j-- // compute the index b < 6! for the // permutation in corner6
    ) {
      var k = 0;
  ...
  CubieCube.prototype.getURtoDF = function () {
    ...
    for (
      var j = 5;
      j > 0;
      j-- // compute the index b < 6! for the // permutation in edge6
    ) {
      var k = 0;
  ...
  CubieCube.prototype.getURtoUL = function () {
    ...
    for (
      var j = 2;
      j > 0;
      j-- // compute the index b < 3! for the // permutation in edge3
    ) {
      var k = 0;
  ...
  CubieCube.prototype.getUBtoDF = function () {
    ...
    for (var j = UR.ordinal(); j <= BR.ordinal(); j++)
      if (
        UB.ordinal() <= this.ep[j].ordinal() &&
        this.ep[j].ordinal() <= DF.ordinal()
      ) {
        a += Cnk(j, x + 1);
        edge3[x++] = this.ep[j];
      }
  ...
  CubieCube.prototype.getURFtoDLB = function () {
    ...
    for (
      var j = 7;
      j > 0;
      j-- // compute the index b < 8! for the permutation in perm
    ) {
      var k = 0;
  ...
  CubieCube.prototype.getURtoBR = function () {
    ...
    for (
      var j = 11;
      j > 0;
      j-- // compute the index b < 12! for the permutation in perm
    ) {
      var k = 0;
      while (perm[j].ordinal() != j) {
        this.rotateLeft(perm, 0, j);
  ...
  CubieCube.prototype.verify = function () {
    ...
    for (var i = 0; i < 12; i++) if (edgeCount[i] != 1) return -2;
    ...
    for (var i = 0; i < 8; i++) if (cornerCount[i] != 1) return -4; // missing corners
    ...
    if ((this.edgeParity() ^ this.cornerParity()) != 0) return -6; // parity error
  ...
  function generateSlice_URFtoDLF_Parity_Prun(FRtoBR_Move, URFtoDLF_Move) {
    var Slice_URFtoDLF_Parity_Prun = new Int8Array(
      ((N_SLICE2 * N_URFtoDLF * N_PARITY) / 2) | 0
    );
  ...
  function generateSlice_URtoDF_Parity_Prun(FRtoBR_Move, URtoDF_Move) {
    var Slice_URtoDF_Parity_Prun = new Int8Array(
      ((N_SLICE2 * N_URtoDF * N_PARITY) / 2) | 0
    );
  ...
  function generateTables() {
    ...
    CoordCube.Slice_URFtoDLF_Parity_Prun = generateSlice_URFtoDLF_Parity_Prun(
      CoordCube.FRtoBR_Move,
      CoordCube.URFtoDLF_Move
    );
    sendProgressMessage('Slice_URFtoDLF_Parity_Prun');
    CoordCube.Slice_URtoDF_Parity_Prun = generateSlice_URtoDF_Parity_Prun(
      CoordCube.FRtoBR_Move,
      CoordCube.URtoDF_Move
    );
    sendProgressMessage('Slice_URtoDF_Parity_Prun');
    CoordCube.Slice_Twist_Prun = generateSlice_Twist_Prun(
      CoordCube.FRtoBR_Move,
      CoordCube.twistMove
    );
    sendProgressMessage('Slice_Twist_Prun');
    CoordCube.Slice_Flip_Prun = generateSlice_Flip_Prun(
      CoordCube.FRtoBR_Move,
      CoordCube.flipMove
    );
  ...
  var Search = {
    ...
    solution: function (facelets, maxDepth, timeOut, useSeparator) {
      ...
      do {
        do {
          if (depthPhase1 - n > this.minDistPhase1[n + 1] && !busy) {
            ...
            do {
              ...
              n != 0 &&
              (this.ax[n - 1] == this.ax[n] || this.ax[n - 1] - 3 == this.ax[n])
            );
          } else busy = false;
        } while (busy);
        ...
        this.slice[n + 1] =
          (CoordCube.FRtoBR_Move[this.slice[n] * 24][mv] / 24) | 0;
        this.minDistPhase1[n + 1] = Math.max(
          CoordCube.getPruning(
            CoordCube.Slice_Flip_Prun,
            CoordCube.N_SLICE1 * this.flip[n + 1] + this.slice[n + 1]
          ),
          CoordCube.getPruning(
            CoordCube.Slice_Twist_Prun,
            CoordCube.N_SLICE1 * this.twist[n + 1] + this.slice[n + 1]
          )
        );
        // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        if (this.minDistPhase1[n + 1] == 0 && n >= depthPhase1 - 5) {
          ...
          if (
            n == depthPhase1 - 1 &&
            (s = this.totalDepth(depthPhase1, maxDepth)) >= 0
          ) {
            if (
              s == depthPhase1 ||
              ...
              return useSeparator
                ? this.solutionToString(s, depthPhase1)
                : this.solutionToString(s);
          }
        }
      } while (true);
    },
    totalDepth: function (depthPhase1, maxDepth) {
      ...
      if (
        (d1 = CoordCube.getPruning(
          CoordCube.Slice_URFtoDLF_Parity_Prun,
          (CoordCube.N_SLICE2 * this.URFtoDLF[depthPhase1] +
            this.FRtoBR[depthPhase1]) *
            2 +
      ...
      this.URtoDF[depthPhase1] =
        CoordCube.MergeURtoULandUBtoDF[this.URtoUL[depthPhase1]][
          this.UBtoDF[depthPhase1]
        ];

      if (
        (d2 = CoordCube.getPruning(
          CoordCube.Slice_URtoDF_Parity_Prun,
          (CoordCube.N_SLICE2 * this.URtoDF[depthPhase1] +
            this.FRtoBR[depthPhase1]) *
            2 +
            this.parity[depthPhase1]
        )) > maxDepthPhase2
      ...
      if ((this.minDistPhase2[depthPhase1] = Math.max(d1, d2)) == 0)
        // already solved
        return depthPhase1;
      ...
      do {
        do {
          if (
            depthPhase1 + depthPhase2 - n > this.minDistPhase2[n + 1] &&
            !busy
          ) {
            if (this.ax[n] == 0 || this.ax[n] == 3) {
              // Initialize next move
            ...
            this.ax[n] == 0 || this.ax[n] == 3
              ? ++this.po[n] > 3
              : (this.po[n] = this.po[n] + 2) > 3
          ) {
            do {
              // increment axis
              if (++this.ax[n] > 5) {
        ...
        this.minDistPhase2[n + 1] = Math.max(
          CoordCube.getPruning(
            CoordCube.Slice_URtoDF_Parity_Prun,
            (CoordCube.N_SLICE2 * this.URtoDF[n + 1] + this.FRtoBR[n + 1]) * 2 +
              this.parity[n + 1]
          ),
          CoordCube.getPruning(
            CoordCube.Slice_URFtoDLF_Parity_Prun,
            (CoordCube.N_SLICE2 * this.URFtoDLF[n + 1] + this.FRtoBR[n + 1]) *
              2 +
              this.parity[n + 1]
          )
        );
