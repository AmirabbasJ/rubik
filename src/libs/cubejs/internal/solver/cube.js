export const Cube = {
  worker: new Worker(new URL('solve.js', import.meta.url)),
  ...
  solveCube: function (
    ...
    if (!Cube.tablesGenerated) {
      throw new Error(
        'Cubes cannot be solved without generating tables first!'
      );
    }
    var eventListener = function (event) {
      if (event.data.type == 'solution' && event.data.cube == cube) {
        if (event.data.result.indexOf('Error') == 0 && errorCallback) {
          ...
          var result = event.data.result.substring(
            0,
            event.data.result.length - 1
          );
          if (resultAsArray) {
